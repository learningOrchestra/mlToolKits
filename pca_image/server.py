...
@app.route('/pca/<parent_filename>', methods=[POST])
def create_pca(parent_filename):
    ...
    try:
        ...
        return jsonify(
            {MESSAGE_RESULT:
                invalid_pca_filename.args[FIRST_ARGUMENT]}),\
...
@app.route('/pca/<filename>', methods=[GET])
def get_image(filename):
    ...
    try:
        request_validator.no_pca_filename_existence_validator(
            filename)
    except Exception as invalid_pca_filename:
        return jsonify(
            {MESSAGE_RESULT:
...
@app.route('/pca/<filename>', methods=[DELETE])
...
if __name__ == "__main__":
    app.run(host=os.environ[PCA_HOST_IP],
            port=int(os.environ[PCA_HOST_PORT]))
